...
function Navbar({handleGetStartedClick}: {handleGetStartedClick: () => void}) {
  const navigate = useNavigate();
  return (
    <nav className='w-full inter-regular py-4 px-4 md:px-12 flex justify-between items-center glassy-navbar relative'>
      ...
      <div className='flex space-x-5 md:space-x-8 justify-center items-center'>
        ...
        <a onClick={handleGetStartedClick} className='text-gray-300 hover:text-[#F97315] text-[16px] md:text-lg'>Get Started</a>
        <button onClick={() => { window.open("https://github.com/ayushflows/Alloan.ai-Stock-Graph-Assesment", "_blank");}} 
          className="px-4 py-[1px] rounded-full relative bg-slate-700 text-white text-[16px] md:text-lg hover:shadow-2xl hover:scale-[1.05] hover:shadow-white/[0.1] transition duration-200 border border-slate-600">
